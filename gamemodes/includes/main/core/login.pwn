...
stock ShowMainMenuDialog(playerid, frame)
{
	...
	switch(frame)
	{
		case 1:
		{
			...
			format(string, sizeof(string), "{FFFFFF}Welcome to SARP, %s.\n\nTen ma ban dang su dung da duoc dang ky, vui long nhap mat khau de dang nhap:", GetPlayerNameEx(playerid));
		...
		case 2:
		{
			...
			format(string, sizeof(string), "{FFFFFF}Welcome to SARP, %s.\n\n{FFFFFF}Ban co the{AA3333} dang ky {FFFFFF}tai khoan tai day:", GetPlayerNameEx(playerid));
		...
		{
			format(titlestring, sizeof(titlestring), "{3399FF}Login - %s", GetPlayerNameEx(playerid));
			format(string, sizeof(string), "{FFFFFF}Mat khau khong hop le!\n\nWelcome to SARP, %s.\n\nTen ma ban dang su dung da duoc dang ky, vui long nhap mat khau de dang nhap:", GetPlayerNameEx(playerid));
			ShowPlayerDialogEx(playerid,MAINMENU,DIALOG_STYLE_PASSWORD,titlestring,string,"Login","Exit");
		}
		case 4:
		{
			format(titlestring, sizeof(titlestring), "{3399FF}Account Locked - %s", GetPlayerNameEx(playerid));
			format(string, sizeof(string), "{FFFFFF}Co so du lieu cua chung toi chi ra rang %s hien dang duoc dang nhap, neu day la loi, vui long lien he voi quan tri vien.", GetPlayerNameEx(playerid));
